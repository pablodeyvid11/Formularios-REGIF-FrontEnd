...
function ModalidadeEnsino({ respostas }: props) {
    ...
    useEffect(() => {
        ...
        respostas.map(dado => {
            if (dado.m == "Sim, tenho um(a).") {
                SimUm++;
            } else if (dado.m == "Sim, tenho alguns.") {
                SimAlguns++;
            } else if (dado.m == "Sim, tenho vários.") {
                SimVarios++;
            } else if (dado.m == "Não, não tenho.") {
                Nao++;
            }
        });
    ...
    return (
        <>
            <h1 className="titulo-modalidade">Você tem professores que obrigam a ligar a câmera e/ou a responder por chat de voz? {`(${respostas.length} respostas)`}</h1>
            <div className="table">
                <VictoryPie
